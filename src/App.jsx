  ...
  function handleLogin(usuario, senha) {
    ...
    if (
      ...
      localStorage.setItem(
        "isAdmin",
        usuarioEncontrado.isAdmin ? "true" : "false"
      );
...
function MainApp({ onLogout, isAdmin, usuarioAtual }) {
  ...
  const processarCodigo = (valorOriginal) => {
    ...
    if (!encontrado) {
      ...
      showNotification(`Nenhum item encontrado para: ${valorOriginal}`, "error");
      return;
    }
      ...
      {notification && (
        <div
          style={{
            ...styles.notificacao,
            backgroundColor:
              notification.type === "success" ? "#4CAF50" : "#e53935",
          }}
        >
          <p style={styles.notificacaoTexto}>{notification.msg}</p>
        </div>
      )}
